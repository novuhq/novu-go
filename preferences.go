...
func newPreferences(rootSDK *Novu, sdkConfig config.SDKConfiguration, hooks *hooks.Hooks) *Preferences {
	return &Preferences{
		rootSDK:          rootSDK,
...
func (s *Preferences) BulkUpdate(ctx context.Context, subscriberID string, bulkUpdateSubscriberPreferencesDto components.BulkUpdateSubscriberPreferencesDto, idempotencyKey *string, opts ...operations.Option) (*operations.SubscribersControllerBulkUpdateSubscriberPreferencesResponse, error) {
	request := operations.SubscribersControllerBulkUpdateSubscriberPreferencesRequest{
		SubscriberID:                       subscriberID,
		IdempotencyKey:                     idempotencyKey,
	...
	opURL, err := utils.GenerateURL(ctx, baseURL, "/v2/subscribers/{subscriberId}/preferences/bulk", request, nil)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	res := &operations.SubscribersControllerBulkUpdateSubscriberPreferencesResponse{
		HTTPMeta: components.HTTPMetadata{
			Request:  req,
			Response: httpRes,
		},
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			...
			var out []components.GetPreferencesResponseDto
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}

			res.GetPreferencesResponseDtos = out
		default:
			rawBody, err := utils.ConsumeRawBody(httpRes)
			if err != nil {
				return nil, err
