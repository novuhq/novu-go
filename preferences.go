...
func (s *Preferences) UpdateGlobal(ctx context.Context, subscriberID string, updateSubscriberGlobalPreferencesRequestDto components.UpdateSubscriberGlobalPreferencesRequestDto, idempotencyKey *string, opts ...operations.Option) (*operations.SubscribersV1ControllerUpdateSubscriberGlobalPreferencesResponse, error) {
	hookCtx := hooks.HookContext{
		Context:        ctx,
	...
	bodyReader, reqContentType, err := utils.SerializeRequestBody(ctx, request, false, false, "UpdateSubscriberGlobalPreferencesRequestDto", "json", `request:"mediaType=application/json"`)
	if err != nil {
		return nil, err
	}
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			...
			var out components.UpdateSubscriberPreferenceGlobalResponseDto
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}

			res.UpdateSubscriberPreferenceGlobalResponseDto = &out
		default:
			rawBody, err := utils.ConsumeRawBody(httpRes)
			if err != nil {
				return nil, err
