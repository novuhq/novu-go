...
type NovuTopicsSubscribers struct {
	sdkConfiguration sdkConfiguration
}

func newNovuTopicsSubscribers(sdkConfig sdkConfiguration) *NovuTopicsSubscribers {
	return &NovuTopicsSubscribers{
...
func (s *NovuTopicsSubscribers) Check(ctx context.Context, externalSubscriberID string, topicKey string, idempotencyKey *string, opts ...operations.Option) (*operations.TopicsControllerGetTopicSubscriberResponse, error) {
	...
	res := &operations.TopicsControllerGetTopicSubscriberResponse{
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			...
			var out components.TopicSubscriberDto
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}

			res.TopicSubscriberDto = &out
		default:
			rawBody, err := utils.ConsumeRawBody(httpRes)
			if err != nil {
				return nil, err
...
func (s *NovuTopicsSubscribers) Remove(ctx context.Context, topicKey string, removeSubscribersRequestDto components.RemoveSubscribersRequestDto, idempotencyKey *string, opts ...operations.Option) (*operations.TopicsControllerRemoveSubscribersResponse, error) {
	hookCtx := hooks.HookContext{
		Context:        ctx,
	...
	opURL, err := utils.GenerateURL(ctx, baseURL, "/v1/topics/{topicKey}/subscribers/removal", request, nil)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	res := &operations.TopicsControllerRemoveSubscribersResponse{
		HTTPMeta: components.HTTPMetadata{
			Request:  req,
			Response: httpRes,
		},
