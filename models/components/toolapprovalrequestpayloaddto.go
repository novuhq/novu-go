// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package components

// To - Novu subscriberId(s) allowed to settle this tool gate when HITL is enabled.
type To struct {
}

type ToolApprovalRequestPayloadDto struct {
	// Unique id for this approval request (matches the AI SDK approvalId).
	ApprovalID string `json:"approvalId"`
	// Id of the tool call awaiting approval.
	ToolCallID string `json:"toolCallId"`
	// Name of the gated tool.
	Name string `json:"name"`
	// Tool input the model proposed.
	Input map[string]any `json:"input,omitempty"`
	// Server-minted approve action id. When omitted, self-hosted tool-approval:* is minted at persist.
	ApproveActionID *string `json:"approveActionId,omitempty"`
	// Server-minted deny action id. When omitted, self-hosted tool-approval:* is minted at persist.
	DenyActionID *string `json:"denyActionId,omitempty"`
	// MCP server name when the gated tool is from an MCP server (for UI labels).
	McpServerName *string `json:"mcpServerName,omitempty"`
	// Novu subscriberId(s) allowed to settle this tool gate when HITL is enabled.
	To *To `json:"to,omitempty"`
	// Attribution label shown on the HITL card.
	From *string `json:"from,omitempty"`
	// Seconds until the HITL tool-gate expires.
	TTLSeconds *float64 `json:"ttlSeconds,omitempty"`
}

func (t *ToolApprovalRequestPayloadDto) GetApprovalID() string {
	if t == nil {
		return ""
	}
	return t.ApprovalID
}

func (t *ToolApprovalRequestPayloadDto) GetToolCallID() string {
	if t == nil {
		return ""
	}
	return t.ToolCallID
}

func (t *ToolApprovalRequestPayloadDto) GetName() string {
	if t == nil {
		return ""
	}
	return t.Name
}

func (t *ToolApprovalRequestPayloadDto) GetInput() map[string]any {
	if t == nil {
		return nil
	}
	return t.Input
}

func (t *ToolApprovalRequestPayloadDto) GetApproveActionID() *string {
	if t == nil {
		return nil
	}
	return t.ApproveActionID
}

func (t *ToolApprovalRequestPayloadDto) GetDenyActionID() *string {
	if t == nil {
		return nil
	}
	return t.DenyActionID
}

func (t *ToolApprovalRequestPayloadDto) GetMcpServerName() *string {
	if t == nil {
		return nil
	}
	return t.McpServerName
}

func (t *ToolApprovalRequestPayloadDto) GetTo() *To {
	if t == nil {
		return nil
	}
	return t.To
}

func (t *ToolApprovalRequestPayloadDto) GetFrom() *string {
	if t == nil {
		return nil
	}
	return t.From
}

func (t *ToolApprovalRequestPayloadDto) GetTTLSeconds() *float64 {
	if t == nil {
		return nil
	}
	return t.TTLSeconds
}
